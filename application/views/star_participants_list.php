...
<div class="container content-area">
	<div class="side-app">
		...
		<div class="page-header">
			<ol class="breadcrumb"><!-- breadcrumb -->
				...
				<li class="breadcrumb-item active" aria-current="page">Star Participants List</li>
			</ol><!-- End breadcrumb -->
			<div class="ml-auto">
				<div class="input-group">
					<a href="<?php echo site_url('Master/star_participants_form'); ?>" class="btn btn-secondary text-white btn-sm" data-toggle="tooltip" title="" data-placement="bottom" data-original-title="Add New">
						<span>
							<i class="fa fa-plus"></i>&nbsp;Star Participants
						</span>
					</a>
				</div>
			</div>
					...
					<table id="example2" class="table table-striped table-bordered text-nowrap" >
						...
						<?php 
							if($star_participants_list){
								foreach($star_participants_list as $program_feedback){
						?>
							<tr id="row<?php echo base64_encode($program_feedback->id); ?>">
								<td>
									<?php if(isset($program_feedback->sequence_no) && $program_feedback->sequence_no!='' && $program_feedback->sequence_no!=null){ 
										echo $program_feedback->sequence_no; 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($program_feedback->field_label) && $program_feedback->field_label!='' && $program_feedback->field_label!=null){ 
										echo $program_feedback->field_label; 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($program_feedback->field_type) && $program_feedback->field_type!='' && $program_feedback->field_type!=null){ 
										echo ucfirst($program_feedback->field_type); 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($program_feedback->is_required) && $program_feedback->is_required==1){ 
										echo 'Yes'; 
									}else{ 
										echo 'No'; 
									} ?>
								...
								<td>
									<?php if(isset($program_feedback->is_active) && $program_feedback->is_active==1){ 
										echo 'Active'; 
									}else{ 
										echo 'Inactive'; 
									} ?>
								...
								<td>
									<?php if(isset($program_feedback->updated_at) && $program_feedback->updated_at!='' && $program_feedback->updated_at!=null){ 
										echo date('d-m-Y',strtotime($program_feedback->updated_at)); 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td style="width:20px;">
									<?php if(isset($program_feedback->is_default) && $program_feedback->is_default==1){  ?>
									...
									<?php }else{ ?>
										<a class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#editField" href="javascript:void(0);" onclick="getDynamicFormFieldData('<?php echo base64_encode($program_feedback->id); ?>');"><i class="fa fa-edit"></i> Edit</a>&nbsp;
										<a class="btn btn-secondary btn-sm" onclick="deleteDynamicField('<?php echo base64_encode($program_feedback->id); ?>');" href="javascript:void(0);"><i class="fa fa-delete"></i> Delete</a>
									<?php } ?>
								</td>										
							</tr>
						<?php 	}										
	...
	function deleteDynamicField(id){
		...
		if(res){
			var url="<?php echo site_url('master/delete_dynamic_field/'); ?>star_participants_form/"+id+'/star_participants_list';
	...
	function getDynamicFormFieldData(id){
		$.ajax({
			...
			data:'tablename=star_participants_form&id='+id,
		...
		$.ajax({
			...
			success:function(response){
				if(response.hasOwnProperty('success_message') && response.success_message){
					...
					if(response.hasOwnProperty('date_validation') && response.date_validation){
						$('#date_validation-error').fadeIn(0);
						$('#date_validation-error').html(response.date_validation);
					}
				}
			}
		});
