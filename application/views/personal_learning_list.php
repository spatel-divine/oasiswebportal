...
<div class="container content-area">
	<div class="side-app">
		...
		<div class="page-header">
			<ol class="breadcrumb"><!-- breadcrumb -->
				...
				<li class="breadcrumb-item active" aria-current="page">Personal Learning List</li>
			</ol><!-- End breadcrumb -->
			<div class="ml-auto">
				<div class="input-group">
					<a href="<?php echo site_url('Master/personal_learning_form'); ?>" class="btn btn-secondary text-white btn-sm" data-toggle="tooltip" title="" data-placement="bottom" data-original-title="Add New">
						<span>
							<i class="fa fa-plus"></i>&nbsp;Personal Reflection (for Fc & CoFc only)
						</span>
					</a>
				</div>
			</div>
			...
			<div class="card-body">
				...
				<div class="table-responsive">
					<table id="example2" class="table table-striped table-bordered text-nowrap" >
						...
						<tbody>
						<?php 
							if($personal_learning_list){
								foreach($personal_learning_list as $personal_learning){
						?>
							<tr id="row<?php echo base64_encode($personal_learning->id); ?>">
								<td>
									<?php if(isset($personal_learning->sequence_no) && $personal_learning->sequence_no!='' && $personal_learning->sequence_no!=null){ 
										echo $personal_learning->sequence_no; 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($personal_learning->field_label) && $personal_learning->field_label!='' && $personal_learning->field_label!=null){ 
										echo $personal_learning->field_label; 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($personal_learning->field_type) && $personal_learning->field_type!='' && $personal_learning->field_type!=null){ 
										echo ucfirst($personal_learning->field_type); 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($personal_learning->is_required) && $personal_learning->is_required==1){ 
										echo 'Yes'; 
									}else{ 
										echo 'No'; 
									} ?>
								...
								<td>
									<?php if(isset($personal_learning->is_active) && $personal_learning->is_active==1){ 
										echo 'Active'; 
									}else{ 
										echo 'Inactive'; 
									} ?>
								...
								<td>
									<?php if(isset($personal_learning->updated_at) && $personal_learning->updated_at!='' && $personal_learning->updated_at!=null){ 
										echo date('d-m-Y',strtotime($personal_learning->updated_at)); 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td style="width:20px;">
									<?php if(isset($personal_learning->is_default) && $personal_learning->is_default==1){  ?>
									...
									<?php }else{ ?>
										<a class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#editField" href="javascript:void(0);" onclick="getDynamicFormFieldData('<?php echo base64_encode($personal_learning->id); ?>');"><i class="fa fa-edit"></i> Edit</a>&nbsp;
										<a class="btn btn-secondary btn-sm" onclick="deleteDynamicField('<?php echo base64_encode($personal_learning->id); ?>');" href="javascript:void(0);"><i class="fa fa-delete"></i> Delete</a>
									<?php } ?>
								</td>										
							</tr>
						<?php 	}										
	...
	function deleteDynamicField(id){
		...
		if(res){
			var url="<?php echo site_url('master/delete_dynamic_field/'); ?>personal_learning_form/"+id+'/personal_learning_list';
	...
	function getDynamicFormFieldData(id){
		$.ajax({
			...
			data:'tablename=personal_learning_form&id='+id,
