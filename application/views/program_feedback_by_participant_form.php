	...
	<div class="side-app">
		...
		<div class="page-header">
			<ol class="breadcrumb"><!-- breadcrumb -->
				...
				<li class="breadcrumb-item active" aria-current="page">Program Feedback by Participant</li>
			</ol><!-- End breadcrumb -->
			<div class="ml-auto">
				<div class="input-group">
					<a href="<?php echo site_url('Master/program_feedback_by_participants_list'); ?>" class="btn btn-info text-white btn-sm" data-toggle="tooltip" title="" data-placement="bottom" data-original-title="Add New">
						<span>
							<i class="fa fa-eye"></i>&nbsp;Program Feedback By Participants List
						</span>
					</a>&nbsp;&nbsp;
			...
			<div class="col-md-12">
				...
				<div class="card">
					<div class="card-body">
						...
						<div id="reviewform">
						<?php
						if(isset($feedback_by_participant) && $feedback_by_participant){
							for($i=0;$i<count($feedback_by_participant);$i+=2){ ?>
								<div class="form-group">
									<div class="row">
										<?php for($j=$i;$j<$i+2;$j++){ 
										if(isset($feedback_by_participant[$j]) && $feedback_by_participant[$j]){
										?>
										<div class="col-lg-6 col-md-12">
											<?php
												$field_id='';
												if(isset($feedback_by_participant[$j]->id) && $feedback_by_participant[$j]->id){ 
													$field_id=$feedback_by_participant[$j]->id;
												}
												$label='';
												if(isset($feedback_by_participant[$j]->field_label) && $feedback_by_participant[$j]->field_label){ 
													$label=$feedback_by_participant[$j]->field_label;
												}
												$field_type='';
												if(isset($feedback_by_participant[$j]->field_type) && $feedback_by_participant[$j]->field_type){ 
													$field_type=$feedback_by_participant[$j]->field_type;
												}
												$field_name='';
												if(isset($feedback_by_participant[$j]->field_name) && $feedback_by_participant[$j]->field_name){ 
													$field_name=$feedback_by_participant[$j]->field_name;
												}
												$is_required=0;
												$is_required_field='';
												$is_required_msg='';
												$required_valid_msg='';
												if(isset($feedback_by_participant[$j]->is_required) && $feedback_by_participant[$j]->is_required==1){ 
													$is_required=1;
												...
												$placeholder='';
												if(isset($feedback_by_participant[$j]->placeholder) && $feedback_by_participant[$j]->placeholder){ 
													$placeholder=$feedback_by_participant[$j]->placeholder;
													
												}
												$comments='';
												if(isset($feedback_by_participant[$j]->comments) && $feedback_by_participant[$j]->comments){ 
													$comments=$feedback_by_participant[$j]->comments;
												}
												$max_upload=1;
												if(isset($feedback_by_participant[$j]->max_upload) && $feedback_by_participant[$j]->max_upload){ 
													$max_upload=$feedback_by_participant[$j]->max_upload;
												}
												$related_table_name='';
												if(isset($feedback_by_participant[$j]->related_table_name) && $feedback_by_participant[$j]->related_table_name){ 
													$related_table_name=$feedback_by_participant[$j]->related_table_name;
												}
												$special_feature='';
												if(isset($feedback_by_participant[$j]->special_feature) && $feedback_by_participant[$j]->special_feature){ 
													$special_feature=$feedback_by_participant[$j]->special_feature;
												}
												$is_readonly='';
												if(isset($feedback_by_participant[$j]->is_readonly) && $feedback_by_participant[$j]->is_readonly==1){ 
													$is_readonly='readonly';
												}
												$date_validation='alldate';
												if(isset($feedback_by_participant[$j]->date_validation) && $feedback_by_participant[$j]->date_validation){ 
													$date_validation=$feedback_by_participant[$j]->date_validation;
												}
											?>
											...
											<?php }else if($field_type=='dropdown'){ 
												...
												<?php }else if($special_feature=='multiple select2'){ 
													if($related_table_name && $related_table_name!='sessionmanagement'){
														...
														$optionlist=getDynamicFieldOption('program_feedback_by_participants_form',$field_id,'dropdown');
													}
												?>
											...
											<?php }else if($field_type=='number'){ 
												$minstr='';
												if(isset($feedback_by_participant[$j]->min_number) && $feedback_by_participant[$j]->min_number!='' && $feedback_by_participant[$j]->min_number!=null){
													$minstr='min="'.$feedback_by_participant[$j]->min_number.'"';
												}
												$maxstr='';
												if(isset($feedback_by_participant[$j]->max_number) && $feedback_by_participant[$j]->max_number!='' && $feedback_by_participant[$j]->max_number!=null){
													$maxstr='max="'.$feedback_by_participant[$j]->max_number.'"';
												}
											?>
											...
											<?php }else if($field_type=='checkbox'){ 
												$optionlist=getDynamicFieldOption('program_feedback_by_participants_form',$field_id,'checkbox',$field_name);
												?>
												<br/>
												<?php echo $optionlist; ?>
												<br/>
											<?php }else if($field_type=='radio'){ 
												$optionlist=getDynamicFieldOption('program_feedback_by_participants_form',$field_id,'radio',$field_name);
											?>
												<br/>
												<?php echo $optionlist; ?>
												<br/>
						...
						<div class="form-group">
							<div class="row">	
								...
								<div class="col-lg-6 col-md-12">
									<label contenteditable="true" class="form-control">Feelings for Program</label>
									<textarea name="program_feeling" class="form-control" placeholder="Enter Feelings for Program"><?php echo set_value('program_feeling');?></textarea>
									<font color="red"><?=form_error("program_feeling");?></font>
								</div>
							</div>
						</div>
						<div class="form-group">
							<div class="row">	
								<div class="col-lg-6 col-md-12">
									<label contenteditable="true" class="form-control">Liked the most & found useful</label>
									<textarea name="found_useful" class="form-control" placeholder="Enter Liked the most & found useful"><?php echo set_value('found_useful'); ?></textarea>
									<font color="red"><?=form_error("found_useful");?></font>
								</div>
								<div class="col-lg-6 col-md-12">
									<label contenteditable="true" class="form-control">Reflections for Facilitators</label>
									<textarea name="reflections_for_facilitator" class="form-control" placeholder="Enter Reflections for Facilitators"><?php echo set_value('reflections_for_facilitator');?></textarea>
									<font color="red"><?=form_error("reflections_for_facilitator");?></font>
								</div>
							</div>
						</div>
						<div class="form-group">
							<div class="row">	
								<div class="col-lg-4 col-md-12">
									...
									<font color="red"><?=form_error("program_rating");?></font>
								</div>
								<div class="col-lg-4 col-md-12">
									<label contenteditable="true" class="form-control">Facilitator Rating</label>
									<div class="card-body">
										<div class="box  box-example-1to10">
										  <div class="box-body">
											<select class="example-1to10" name="facilitator_rating" autocomplete="off">
											  <option value="1">1</option>
											  <option value="2">2</option>
											  <option value="3">3</option>
											  <option value="4" selected>4</option>
									...
									<font color="red"><?=form_error("facilitator_rating");?></font>
								</div>
								<div class="col-lg-4 col-md-12">
									...
									<font color="red"><?=form_error("venue_rating");?></font>
								</div>
							</div>
						</div>
						<div class="form-group">
							<div class="row">	
								<div class="col-lg-4 col-md-12">
									<label contenteditable="true" class="form-control">What you did not like & why</label>
									<textarea name="did_not_like" class="form-control" placeholder="Enter What you did not like & why"><?php echo set_value('did_not_like');?></textarea>
									<font color="red"><?=form_error("did_not_like");?></font>
								</div>
								<div class="col-lg-4 col-md-12">
									<label contenteditable="true" class="form-control">Suggestions for Improvements</label>
									<textarea name="suggestions" class="form-control" placeholder="Enter Suggestions for Improvements"><?php echo set_value('suggestions');?></textarea>
									<font color="red"><?=form_error("suggestions");?></font>
								</div>
								<div class="col-lg-4 col-md-12">
									<label contenteditable="true" class="form-control">Any other note</label>
									<textarea name="any_note" class="form-control" placeholder="Enter Any other note"><?php echo set_value('any_note');?></textarea>
									<font color="red"><?=form_error("any_note");?></font>
								</div>
							</div>
						</div>
						...
						<div id="add_fields_form" style="display:none;">
							<form id="program_feedback_by_participant_form" name="program_feedback_by_participant_form" method="post" action="<?php echo base_url('master/program_feedback_by_participant_form');?>">
	                           <?php //include('dynamic_field_generation.php');?>
...
<script type="text/javascript">
	$('#program_feedback_by_participant_form').validate();
</script>