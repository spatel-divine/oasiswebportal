...
<div class="container content-area">
	<div class="side-app">
		...
		<div class="page-header">
			<ol class="breadcrumb"><!-- breadcrumb -->
				...
				<li class="breadcrumb-item active" aria-current="page">Impact on Character Traits List</li>
			</ol><!-- End breadcrumb -->
			<div class="ml-auto">
				<div class="input-group">
					<a href="<?php echo site_url('Master/impact_on_character_traits_form'); ?>" class="btn btn-secondary text-white btn-sm" data-toggle="tooltip" title="" data-placement="bottom" data-original-title="Add New">
						<span>
							<i class="fa fa-plus"></i>&nbsp;Impact on Character Traits
						</span>
					</a>
				</div>
			</div>
			...
			<div class="card-body">
				...
				<div class="table-responsive">
					<table id="example2" class="table table-striped table-bordered text-nowrap" >
						...
						<tbody>
						<?php 
							if($impact_on_character_traits_list){
								foreach($impact_on_character_traits_list as $impact_on_character_traits){
						?>
							<tr id="row<?php echo base64_encode($impact_on_character_traits->id); ?>">
								<td>
									<?php if(isset($impact_on_character_traits->sequence_no) && $impact_on_character_traits->sequence_no!='' && $impact_on_character_traits->sequence_no!=null){ 
										echo $impact_on_character_traits->sequence_no; 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($impact_on_character_traits->field_label) && $impact_on_character_traits->field_label!='' && $impact_on_character_traits->field_label!=null){ 
										echo $impact_on_character_traits->field_label; 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($impact_on_character_traits->field_type) && $impact_on_character_traits->field_type!='' && $impact_on_character_traits->field_type!=null){ 
										echo ucfirst($impact_on_character_traits->field_type); 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($impact_on_character_traits->is_required) && $impact_on_character_traits->is_required==1){ 
										echo 'Yes'; 
									}else{ 
										echo 'No'; 
									} ?>
								...
								<td>
									<?php if(isset($impact_on_character_traits->is_active) && $impact_on_character_traits->is_active==1){ 
										echo 'Active'; 
									}else{ 
										echo 'Inactive'; 
									} ?>
								...
								<td>
									<?php if(isset($impact_on_character_traits->updated_at) && $impact_on_character_traits->updated_at!='' && $impact_on_character_traits->updated_at!=null){ 
										echo date('d-m-Y',strtotime($impact_on_character_traits->updated_at)); 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td style="width:20px;">
									<?php if(isset($impact_on_character_traits->is_default) && $impact_on_character_traits->is_default==1){  ?>
									...
									<?php }else{ ?>
										<a class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#editField" href="javascript:void(0);" onclick="getDynamicFormFieldData('<?php echo base64_encode($impact_on_character_traits->id); ?>');"><i class="fa fa-edit"></i> Edit</a>&nbsp;
										<a class="btn btn-secondary btn-sm" onclick="deleteDynamicField('<?php echo base64_encode($impact_on_character_traits->id); ?>');" href="javascript:void(0);"><i class="fa fa-delete"></i> Delete</a>
									<?php } ?>
								</td>										
							</tr>
						<?php 	}										
	...
	function deleteDynamicField(id){
		...
		if(res){
			var url="<?php echo site_url('master/delete_dynamic_field/'); ?>impact_on_character_traits_form/"+id+'/impact_on_character_traits_list';
	...
	function getDynamicFormFieldData(id){
		$.ajax({
			...
			data:'tablename=impact_on_character_traits_form&id='+id,
