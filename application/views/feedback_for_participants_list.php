...
<div class="container content-area">
	<div class="side-app">
		...
		<div class="page-header">
			<ol class="breadcrumb"><!-- breadcrumb -->
				...
				<li class="breadcrumb-item active" aria-current="page">Feedback for Participants (by Facilitator) List</li>
			</ol><!-- End breadcrumb -->
			<div class="ml-auto">
				<div class="input-group">
					<a href="<?php echo site_url('Master/feedback_for_participants_form'); ?>" class="btn btn-secondary text-white btn-sm" data-toggle="tooltip" title="" data-placement="bottom" data-original-title="Add New">
						<span>
							<i class="fa fa-plus"></i>&nbsp;Feedback for Participants (by Facilitator)
						</span>
					</a>
				</div>
			</div>
			...
			<div class="card-body">
				...
				<div class="table-responsive">
					<table id="example2" class="table table-striped table-bordered text-nowrap" >
						...
						<tbody>
						<?php 
							if($feedback_participants_list){
								foreach($feedback_participants_list as $feedback_participants){
						?>
							<tr id="row<?php echo base64_encode($feedback_participants->id); ?>">
								<td>
									<?php if(isset($feedback_participants->sequence_no) && $feedback_participants->sequence_no!='' && $feedback_participants->sequence_no!=null){ 
										echo $feedback_participants->sequence_no; 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($feedback_participants->field_label) && $feedback_participants->field_label!='' && $feedback_participants->field_label!=null){ 
										echo $feedback_participants->field_label; 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($feedback_participants->field_type) && $feedback_participants->field_type!='' && $feedback_participants->field_type!=null){ 
										echo ucfirst($feedback_participants->field_type); 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td>
									<?php if(isset($feedback_participants->is_required) && $feedback_participants->is_required==1){ 
										echo 'Yes'; 
									}else{ 
										echo 'No'; 
									} ?>
								...
								<td>
									<?php if(isset($feedback_participants->is_active) && $feedback_participants->is_active==1){ 
										echo 'Active'; 
									}else{ 
										echo 'Inactive'; 
									} ?>
								...
								<td>
									<?php if(isset($feedback_participants->updated_at) && $feedback_participants->updated_at!='' && $feedback_participants->updated_at!=null){ 
										echo date('d-m-Y',strtotime($feedback_participants->updated_at)); 
									}else{ 
										echo 'N/A'; 
									} ?>
								</td>
								<td style="width:20px;">
									<?php if(isset($feedback_participants->is_default) && $feedback_participants->is_default==1){  ?>
									...
									<?php }else{ ?>
										<a class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#editField" href="javascript:void(0);" onclick="getDynamicFormFieldData('<?php echo base64_encode($feedback_participants->id); ?>');"><i class="fa fa-edit"></i> Edit</a>&nbsp;
										<a class="btn btn-secondary btn-sm" onclick="deleteDynamicField('<?php echo base64_encode($feedback_participants->id); ?>');" href="javascript:void(0);"><i class="fa fa-delete"></i> Delete</a>
									<?php } ?>
								</td>										
							</tr>
						<?php 	}										
	...
	function deleteDynamicField(id){
		...
		if(res){
			var url="<?php echo site_url('master/delete_dynamic_field/'); ?>feedback_for_participants_form/"+id+'/feedback_participants_list';
	...
	function getDynamicFormFieldData(id){
		$.ajax({
			...
			data:'tablename=feedback_for_participants_form&id='+id,
